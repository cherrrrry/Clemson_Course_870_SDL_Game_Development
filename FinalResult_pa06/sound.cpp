  ...
  channels() 
{
  ...
  if (!music) throw string("Couldn't load back.mp3")+Mix_GetError();

  startMusic();
  sounds.push_back( Mix_LoadWAV("sound/shoot.wav") );
  sounds.push_back( Mix_LoadWAV("sound/butterflyExplode.wav") );
  sounds.push_back( Mix_LoadWAV("sound/angelExplode.wav") );
  sounds.push_back( Mix_LoadWAV("sound/playerExplode.wav") );
