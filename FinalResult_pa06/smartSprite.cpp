...
SmartSprite::SmartSprite(const std::string& name,const Drawable& p):
 ScaledMultiSprite(name), 
...
SmartSprite::SmartSprite(const std::string& name, 
            const Vector2f& pos, const Drawable& p) :
 ScaledMultiSprite(name, pos, 
...
void SmartSprite::update(Uint32 ticks) {
  ScaledMultiSprite::update(ticks);
