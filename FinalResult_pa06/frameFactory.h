...
public:
  ...
  std::vector<Frame*> getScaledFrames(const std::string& name,float scale);
private:
  ...
  std::map<std::string, std::vector<SDL_Surface*> > scaledMultiSurfaces;
  ...
  std::map<std::string, std::vector<Frame*> > scaledMultiFrames;
